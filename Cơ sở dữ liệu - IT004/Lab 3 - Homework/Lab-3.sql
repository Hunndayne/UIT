...
SELECT ChuyenGia.HoTen, COALESCE(DuAn.TenDuAn, 'Không có dự án') AS TenDuAn
FROM ChuyenGia
...
AND NOT EXISTS (
    SELECT 1
    FROM ChuyenGia_KyNang AS CK2
    INNER JOIN KyNang AS K2 ON CK2.MaKyNang = K2.MaKyNang
    WHERE CK2.MaChuyenGia = ChuyenGia.MaChuyenGia AND K2.TenKyNang = 'Python'
);
